...
func test_cooldown_starts_on_validate_only_and_cancel_keeps_spell_available() -> void:
	var ability = ShieldSlamAbilityScript.new()
...
func test_shield_slam_applies_scaled_damage_and_stun_meta() -> void:
	...
	var cast_success: bool = ability.validate_cast(caster, {"target": target})
	...
	assert_gt(target.stun_remaining, 0.0, "Le stun natif doit être appliqué")
...
func test_whirl_slash_hits_each_enemy_once_in_radius() -> void:
	...
	var ability = WhirlSlashAbilityScript.new()
	assert_true(ability.begin_targeting())
	var cast_success: bool = ability.validate_cast(caster, {"targets": [in_range_enemy, far_enemy]})
